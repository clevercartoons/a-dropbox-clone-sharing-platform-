(function($) {
    ...
    let storageTestConnectionBtn = $('.storage-test-connection');
    storageTestConnectionBtn.on('click', function(e) {
        e.preventDefault();
        const provider = $(this).data('provider');
        storageTestConnectionBtn.prop("disabled", true);
        $('.' + provider + '-button-spinner').removeClass('d-none');
        $.ajax({
            url: BASE_URL + '/settings/storage/connect/' + provider,
            type: 'POST',
            dataType: "JSON",
            success: function(response) {
                if ($.isEmptyObject(response.error)) {
                    storageTestConnectionBtn.prop("disabled", false);
                    $('.' + provider + '-button-spinner').addClass('d-none');
    ...
    let addonDisableAction = $('#addonDisableAction');
    addonDisableAction.on('change', function() {
        if ($(this).prop('checked') == false) {
            Swal.fire({
                icon: "warning",
                title: "Attention",
                text: "If the addon disabled and you still using it, it will not work make sure you are not using it anymore.",
            });
        }
    });
    ...
    editSubscription.on('click', function(e) {
        e.preventDefault();
        const id = $(this).data('id');
        $('#editSubscriptionForm').attr('action', '');
        $('#subscriptionStatus').find(':selected').prop('selected', false);
        $('#subscriptionPlan').find(':selected').prop('selected', false);
        $('#subscriptionExpirtAt').attr('value', '');
        $.ajax({
            url: BASE_URL + '/subscriptions/' + id + '/edit',
            type: 'GET',
            dataType: 'JSON',
            success: function(response) {
                if ($.isEmptyObject(response.error)) {
                    $('#editSubscriptionForm').attr('action', response.update_link);
                    $('#subscriptionStatus option[value=' + response.status + ']').prop('selected', true);
                    $('#subscriptionPlan option[value=' + response.plan + ']').prop('selected', true);
                    $('#subscriptionExpirtAt').attr('value', response.expiry_at);
