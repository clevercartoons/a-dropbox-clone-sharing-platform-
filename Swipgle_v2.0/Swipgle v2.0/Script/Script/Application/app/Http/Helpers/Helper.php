...
function vDate($date)
{
    $format = Carbon::parse($date)->format(dateFormatsArray()[settings('date_format')]);
    return $format;
}
...
function vIpInfo()
{
    ...
    if (Cache::has($ip)) {
        ...
        $ipInfo = (object) json_decode(file_get_contents("http://ip-api.com/json/{$ip}?fields={$fields}"), true);
...
{
    $button = '<div class="plan-action"></div>';
    if (auth()->user() or $plan->auth) {
        if (auth()->user() && !is_null(userAuthInfo()->subscription) &&
            ...
            $confirmClass = (Auth::user()) ? 'class="vr__confirm__action__form"' : '';
            $url = route('subscribe', [hashid($plan->id), $type]);
            $token = csrf_token();
            $button = '<div class="plan-action"><form action="' . $url . '" method="POST"><input type="hidden" name="_token" value="' . $token . '"><button type="submit" ' . $confirmClass . '>' . $buttonText . '</button></form></div>';
        }
    }
    return $button;
}
...
function planBadge($plan_id)
{
    ...
    if ($plan->price == 0) {
        if (Auth::user() && !is_null(userAuthInfo()->subscription) && $plan->id == userAuthInfo()->subscription->plan_id) {
            $class = '<span class="current-text">' . lang('Your plan', 'plans') . '</span>';
        } else {
            $class = '<span class="free-text">' . lang('free') . '</span>';
        }
    } elseif ($plan->featured_plan) {
        if (Auth::user() && !is_null(userAuthInfo()->subscription) && $plan->id == userAuthInfo()->subscription->plan_id) {
            $class = '<span class="current-text">' . lang('Your plan', 'plans') . '</span>';
        } else {
            $class = '<span class="featured-text">' . lang('Featured', 'plans') . '</span>';
        }
    } elseif (Auth::user() && !is_null(userAuthInfo()->subscription) && $plan->id == userAuthInfo()->subscription->plan_id) {
        $class = '<span class="current-text">' . lang('Your plan', 'plans') . '</span>';
    }
