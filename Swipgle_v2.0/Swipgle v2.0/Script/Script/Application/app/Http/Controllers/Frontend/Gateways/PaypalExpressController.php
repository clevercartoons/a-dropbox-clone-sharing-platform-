...
class PaypalExpressController extends Controller
{
    ...
    public static function process($trx)
    {
        ...
        $gatewayFees = ($trx->plan_price * paymentGateway('paypal_express')->fees) / 100;
        ...
        $redirectUrls->setReturnUrl(route('ipn.paypal_express'))
            ->setCancelUrl(route('user.checkout.index', $trx->checkout_id));
    ...
    public function ipn(Request $request)
    {
        ...
        try {
            ...
            $trx = \App\Models\Transaction::where([['user_id', userAuthInfo()->id], ['transaction_id', $invoice_number], ['status', 0]])->first();
            if (is_null($trx)) {
