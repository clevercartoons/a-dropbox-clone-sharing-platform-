...
class DownloadController extends Controller
{
    ...
    public function unlockTransfer(Request $request, $link)
    {
        ...
        if (is_null($transfer)) {
            toastr()->error(lang('Transfer not found', 'download'));
            return back();
        }
        ...
        if (Hash::check($request->password, $transfer->password)) {
            ...
            toastr()->error(lang('Incorrect password', 'download'));
            return back();
        }
    }
    ...
    public function requestDownloadLink(Request $request, $link)
    {
        $transfer = Transfer::where([['link', $link], ['expiry_at', '>', Carbon::now()], ['status', 1]])
            ->orWhere([['link', $link], ['expiry_at', null], ['status', 1]])->first();
        if (is_null($transfer)) {
            return response()->json(['error' => lang('Transfer not found', 'download')]);
        }
        if (!is_null($transfer->password) && !Session::has($transfer->link . '_' . sha1($transfer->password))) {
            return response()->json(['error' => lang('Unauthorized access', 'download')]);
        }
        ...
        if (is_null($transferFile)) {
            return response()->json(['error' => lang('Requested file not exists', 'download')]);
        }
    ...
    public function download($link, $file_id)
    {
        ...
        $download = $handler::download($transferFile->path);
        $downloadResponse = json_decode($download);
        if (isset($downloadResponse->status) && $downloadResponse->status == "error") {
            toastr()->error($downloadResponse->msg);
            return redirect()->route('transfer.download.index', $transfer->link);
        }
        ...
        if ($transferFile->storageProvider->symbol != "local") {
            return redirect($downloadResponse->link);
        } else {
            return $download;
        }
    }
    ...
    public function downloadAll($link)
    {
        ...
        if ($transfer->storageProvider->symbol != "local") {
            toastr()->error(lang('Download all not supported on this transfer', 'download'));
            return redirect()->route('transfer.download.index', $transfer->link);
        }
        ...
        if ($transferFiles->count() > 1) {
            ...
            foreach ($transferFiles as $transferFile) {
                $transferFile->increment('downloads', 1);
                $files[] = storage_path('app/public/' . $transferFile->path);
            }
