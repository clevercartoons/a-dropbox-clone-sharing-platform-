        ...
        try {
            ...
            if ($request->has('password')) {
                if ($subscription->plan->transfer_password) {
                    ...
                    return response()->json(['error' => lang('Setting password feature not available for your subscription', 'upload zone')]);
                }
            }
            ...
            if ($request->has('download_notify') or $request->has('expiry_notify')) {
                if ($subscription->plan->transfer_notify) {
                    ...
                    return response()->json(['error' => lang('The notify on download and expiry feature not available for your subscription', 'upload zone')]);
                }
            }
            if ($request->has('expiry_at')) {
                if (!$subscription->plan->transfer_expiry) {
                    return response()->json(['error' => lang('Setting expiry date feature not available for your subscription', 'upload zone')]);
                }
            ...
            if ($createTransfer) {
                ...
                return response()->json([
                    'transfer_expiry_on' => ($createTransfer->expiry_at) ? vDate($createTransfer->expiry_at) : '',
                    'transfer_download_link' => route('transfer.download.index', $createTransfer->link),
                    'view_transfer_link' => (Auth::user()) ? route('user.transfers.show', $createTransfer->unique_id) : null,
                    'facebook_share_link' => 'https://www.facebook.com/sharer/sharer.php?u=' . route('transfer.download.index', $createTransfer->link),
                    'twitter_share_link' => 'https://twitter.com/intent/tweet?text=' . route('transfer.download.index', $createTransfer->link),
                    'linkedin_share_link' => 'https://www.linkedin.com/shareArticle?mini=true&url=' . route('transfer.download.index', $createTransfer->link),
                    'whatsapp_share_link' => 'https://wa.me/?text=' . route('transfer.download.index', $createTransfer->link),
                    'transfer_size' => $transferSize,
                    'subscription' => [
                        'used_space' => subscription()->storage->used_space,
                        'used_percentage' => subscription()->storage->used_percentage,
                    ],
                    'rating' => (is_null($rating)) ? false : true,
                ]);
            }
        } catch (Exception $e) {
