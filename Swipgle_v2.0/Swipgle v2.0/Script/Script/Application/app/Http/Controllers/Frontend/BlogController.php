...
class BlogController extends Controller
{
    public function index(Request $request, $slug = null)
    {
        ...
        if ($request->has('q')) {
            ...
            if ($blogCategory) {
                ...
                $blogArticles = BlogArticle::where([['lang', getLang()], ['category_id', $blogCategory->id]])->with(['blogCategory', 'admin'])->withCount('comments')->latest()->paginate(5);
                $blogCategory = $blogCategory->name;
            } else {
                return redirect()->route('blog.index');
            }
        } else {
            $blogArticles = BlogArticle::where('lang', getLang())->with(['blogCategory', 'admin'])->withCount('comments')->latest()->paginate(5);
        }
    ...
    public function comment(Request $request, $slug)
    {
        ...
        if ($createComment) {
            $title = "New comment waiting review";
