...
class TicketController extends Controller
{
    public function index(Request $request, $status = null)
    {
        if ($request->input('search')) {
            ...
            if ($status) {
                $tickets = SupportTicket::where([['status', $this->ticketStatus($status)], ['user_id', userAuthInfo()->id]])->with('user')->orderbyDesc('id')->paginate(12);
            } else {
                $tickets = SupportTicket::where('user_id', userAuthInfo()->id)->orderbyDesc('updated_at')->paginate(12);
            }
        }
