...
class SubscriptionController extends Controller
{
    ...
    {
        $transactions = Transaction::where([['user_id', userAuthInfo()->id], ['status', '!=', 0]])
            ->with('plan')
            ->orderbyDesc('id')
            ->paginate(10);
    ...
    {
        $transaction = Transaction::where([['transaction_id', $transaction_id], ['user_id', userAuthInfo()->id], ['status', '!=', 0]])
            ->with(['plan', 'gateway'])
            ->firstOrFail();
