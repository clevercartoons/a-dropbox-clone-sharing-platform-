...
class SubscriptionManager
{
    ...
    public static function registredUserSubscriptionDetails($id)
    {
        ...
        $data = (object) [
            ...
            'remining_days' => Carbon::now()->diffInDays(Carbon::parse($user->subscription->expiry_at), false),
            'expired_at' => static::countReminingTimeByDate($user->subscription->expiry_at),
            'is_expired' => (Carbon::now() > $user->subscription->expiry_at) ? true : false,
            "plan" => (object) [
                'id' => $user->subscription->plan->id,
                'name' => $user->subscription->plan->name,
                'interval' => $user->subscription->plan->interval,
                'interval_name' => ($user->subscription->plan->interval) ? lang('yearly', 'plans') : lang('monthly', 'plans'),
        ...
        if (!is_null($plan)) {
            $data = (object) [
                ...
                "plan" => (object) [
                    ...
                    'interval_name' => ($plan->interval) ? lang('yearly', 'plans') : lang('monthly', 'plans'),
                    ...
                    'transfer_interval_days' => ($plan->transfer_interval) ? ($plan->transfer_interval > 1) ? $plan->transfer_interval . ' ' . lang('days') : $user->subscription->plan->transfer_interval . ' ' . lang('day') : lang('Unlimited time', 'user'),
