...
@section('title', __('Transfers | Guests | #') . $transfer->unique_id)
@section('back', route('admin.transfers.guests.index'))
@section('content')
    ...
    <div class="transfer">
        ...
        @endif
        <div class="row g-3">
            <div class="{{ isExpiry($transfer->expiry_at) || !$transfer->status ? 'col-lg-12' : 'col-lg-6' }}">
                <div class="custom-list card shadow-sm border-0 h-100">
                    ...
                    <ul class="custom-list-group list-group list-group-flush">
                        ...
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            <span><strong>{{ __('Guest Ip') }}</strong></span>
                            <span><a href="{{ route('admin.users.logsbyip', $transfer->ip) }}"><i
                                        class="fas fa-map-marker-alt me-2"></i>{{ $transfer->ip }}</a></span>
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            ...
                            <div class="list">
                                @foreach ($transfer->transferFiles as $transferFile)
                                    <div class="list-item d-flex justify-content-between">
                                        ...
                                        <span>
                                            <span class="me-3"><a
                                                    href="{{ route('admin.transfers.guests.download', [$transfer->unique_id, hashid($transferFile->id)]) }}"><i
                                            ...
                                            @if (!isExpiry($transfer->expiry_at) && count($transfer->transferFiles) > 1)
                                                <form class="d-inline"
                                                    action="{{ route('admin.transfers.guests.deleteFile', [$transfer->unique_id, hashid($transferFile->id)]) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('DELETE')
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <form action="{{ route('admin.transfers.guests.update', $transfer->unique_id) }}" method="POST">
                            @csrf
